...
>
> **תוספת (2026-09-13):** המפתח סיפק קבצי אודיו ובחר להחליף חלק מהצלילים
> המסונתזים בקבצים אמיתיים, ובכך לבטל בהדרגה את הסעיף הראשון של החלטה #1
> לעיל:
> - **מוזיקת רקע** — קובץ `background.wav` (WAV לא דחוס, ~63MB) הומר ל-MP3
>   128kbps (~3.4MB) ומתנגן בלולאה דרך `musicGain`.
> - **לייזר** — קובץ `laser.mp3` שסופק (2.4s, כולל ~2s שקט בסוף) קוצץ
>   ל-0.5s ומקודד מחדש ל-MP3 mono 128kbps (~9KB). נטען פעם אחת ל-AudioBuffer
>   ומתנגן דרך BufferSource חדש בכל ירייה, כדי שאש רצופה תוכל לחפוף (אין
>   Cooldown ל-`shoot`).
>
> **אפקט הקול היחיד שנותר מסונתז** הוא הפיצוצים. ההמרות ל-MP3 נשמרות קטנות
> כדי שלא יכבידו על ה-Precache של ה-Service Worker (M8/M9 בנויים סביב
> אפליקציה קלה ל-Offline). ראו `src/services/audioService.ts`,
> `src/game/gameConfig.ts` ו-`vite.config.ts`.
